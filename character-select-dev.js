...
window.addEventListener('load',()=>{
  ...
  init();const s=net()?.socket;if(s){if(typeof s.onAny==='function')s.onAny((e,p)=>{if(p?.state)sync(p.state)});s.on('freshCharacterChosen',p=>sync(p?.state));}new MutationObserver(()=>{if(document.body.classList.contains('fresh-game-ready')&&window.FRESH_NET?.lastState?.characterMode==='on'&&!document.body.dataset.pingCharacter)document.getElementById('characterChoice')?.classList.add('open')}).observe(document.body,{attributes:true,attributeFilter:['class']});});
})();